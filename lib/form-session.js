...
function createUserSession(userId, response) {
	return new Session({ owner: userId }).save().then(function (session) {
		var secure = response.req.headers['x-forwarded-protocol'] !== 'http';

		response.setHeader('Set-Cookie',
			util.format(
				't=%d:%s; Max-Age=%d; Path=/; %s HttpOnly',
				session.id, sign('' + session.id), userMaxAge, secure ? 'Secure;' : ''
			)
		);
	});
}
...
function removeSession(sessionId) {
	return new Session({ id: sessionId }).destroy();
}
...
function middleware(request, response, next) {
	...
	User.query(function (q) {
		q.innerJoin('sessions', 'sessions.owner', '=', 'users.id')
		 .where('sessions.id', '=', token.sessionId);
	}).fetch()
		.then(function (user) {
			if (!user) {
